...
const roofMap = baseMap.map((row, y) =>
...
const charMap: Record<string, Partial<Cell>> = {
...
// Track entity positions and stats
...
function astar(start: {x: number, y: number, z: number}, goal: {x: number, y: number, z: number}): {x: number, y: number, z: number, cost: number}[] | null {
  type Node = {
    x: number;
    y: number;
    z: number;
...
function isWalkableFloor(floor: FloorType): boolean {
  return floor === FloorType.Floor || floor === FloorType.StairsDown;
}
function isWalkableWall(wall: WallType): boolean {
  return wall === WallType.Air || wall === WallType.StairsUp;
}

// Helper for movement cost (including stairs)
function getMoveCost(from: {x: number, y: number, z: number}, to: {x: number, y: number, z: number}): number {
...
function drawMap(zombies: Zombie[], player: {x: number, y: number, z: number} | null, zLevel: number) {
...
function renderState(tickMsg?: string) {
  console.clear();
  if (tickMsg) {
    console.log(tickMsg);
  }
  ...
  zombies.forEach((z, i) => {
    ...
    // Awareness
    const seen = getVisibleObjects(z, grid, zombies, player);
    const heard = getHeardObjects(z, grid, zombies, player);
...
function parseDirection(numKey: string): {x: number, y: number, z: number} | null {
  switch (numKey) {
...
function startPlayerMovement(player: Player, direction: {x: number, y: number, z: number} | null, grid: Grid) {
  if (direction) {
    ...
    if (grid.isValidMove(dx, dy, dz)) {
      ...
      if (player.moveProgress < moveCost) {
        ...
        player.x = dx; player.y = dy; player.z = dz;
        player.moveProgress -= moveCost;
